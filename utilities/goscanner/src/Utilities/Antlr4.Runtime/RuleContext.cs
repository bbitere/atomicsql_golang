...
namespace Antlr4.Runtime
{
    ...
    public class RuleContext : IRuleNode
    {
        ...
        static Dictionary<string, int> dictAlt = new Dictionary<string, int>(); 
        public int? GetCacheAlt( object[] keys )
        {
            var key = "";
            ...
            if( dictAlt.TryGetValue( key, out var value ) )
                return value;

            return null;
        }
        public void SetCacheAlt( int alt, object[] keys )
        {
            var key = "";
            foreach( var o in keys)
            {
