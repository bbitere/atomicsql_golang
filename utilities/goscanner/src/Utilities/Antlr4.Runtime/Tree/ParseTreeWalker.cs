...
namespace Antlr4.Runtime.Tree
{
    public class ParseTreeWalker
    {
        ...
        public virtual void Walk(IParseTreeListener listener, IParseTree t)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                var item = r.GetChild(i);
                Walk(listener, item );

                if( i == 0 && item is IRuleNode ) 
                {    //I did this modificatiaon because for correct behavior of method(arg), I need to know first the method name in sql convert
                    EnterAfterFirstChildRule(listener, r);
                }
            }
            ExitRule(listener, r);
        }
        ...
        protected internal virtual void EnterAfterFirstChildRule(IParseTreeListener listener, IRuleNode r)
        {
            ParserRuleContext ctx = (ParserRuleContext)r.RuleContext;
            listener.AfterFirstChildEveryRule(ctx);
            //ctx.EnterRule(listener);
        }
