...
public partial class SqlConvert
{
    ...
    public override void EnterPackageClause(GoParser.PackageClauseContext context)
    {
        ...
        { 
            ...
            {
                if (m_targetOutputFile.Length == 0)
                    headerLevelComments = headerLevelComments.TrimStart();

                m_targetOutputFile.Append(headerLevelComments);

                if (!EndsWithLineFeed(headerLevelComments))
                    m_targetOutputFile.AppendLine();

                if (!Options.ExcludeHeaderComments && !EndsWithDuplicateLineFeed(m_targetOutputFile.ToString()))
                    m_targetOutputFile.AppendLine();
            }

            if (!Options.ExcludeHeaderComments)
            {
                m_targetOutputFile.AppendLine($"// package {Package} -- go2cs converted at {DateTime.UtcNow:yyyy MMMM dd HH:mm:ss} UTC");

                if (!PackageImport.Equals("main"))
                    m_targetOutputFile.AppendLine($"// import \"{PackageImport}\" ==> using {PackageUsing}");

                m_targetOutputFile.AppendLine($"// Original source: {SourceFileName}");
            }
            ...
            m_targetOutputFile.Append(m_namespaceHeader);
