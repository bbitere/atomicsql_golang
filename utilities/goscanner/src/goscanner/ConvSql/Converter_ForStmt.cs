    ...
    public override void EnterForStmt(GoParser.ForStmtContext context)
    {
        ...
        { 
            ...
            if (context.expression() is not null || forClause is not null && forClause.simpleStmt()?.Length == 0 || context.children.Count < 3)
            {
                ...
                m_targetOutputFile.Append($"{Spacing()}while ({string.Format(ForExpressionMarker, m_forExpressionLevel)})");
            ...
            else if (forClause is not null)
            {
                ...
                if (hasInitStatement)
                {
                    ...
                    if (isRedeclared || !useForStyleStatement && simpleInitStatement.shortVarDecl() is not null)
                    {
                        ...
                        m_targetOutputFile.Append(OpenRedeclaredVariableBlock(simpleInitStatement.shortVarDecl().identifierList(), m_forExpressionLevel));
                    }
                }
                    ...
                    hasPostStatement && (simplePostStatement.incDecStmt() is not null || simplePostStatement.expressionStmt() is not null))
                {
                    ...
                    m_targetOutputFile.Append($"{Spacing()}for ({string.Format(ForInitStatementMarker, m_forExpressionLevel)}; {string.Format(ForExpressionMarker, m_forExpressionLevel)}; {string.Format(ForPostStatementMarker, m_forExpressionLevel)})");
                ...
                {
                    if (hasInitStatement)
                        m_targetOutputFile.Append(string.Format(ForInitStatementMarker, m_forExpressionLevel));
                ...
                if (rangeClause is not null)
                {
                    if (rangeClause.identifierList() is not null)
                    {
                        ...
                        if (isRedeclared)
                        {
                            ...
                            // Handle storing of current values of any redeclared variables
                            m_targetOutputFile.Append(OpenRedeclaredVariableBlock(rangeClause.identifierList(), m_forExpressionLevel));
                        }
                    }

                    m_targetOutputFile.Append($"{Spacing()}foreach ({string.Format(ForRangeExpressionsMarker, m_forExpressionLevel)} in {string.Format(ForExpressionMarker, m_forExpressionLevel)})");
    ...
    public override void ExitForStmt(GoParser.ForStmtContext context)
    {
        ...
        { 
            ...
            if (context.expression() is not null || forClause is not null && forClause.simpleStmt()?.Length == 0 || context.children.Count < 3)
            {
                ...
                m_targetOutputFile.Replace(string.Format(ForExpressionMarker, m_forExpressionLevel), expression?.Text ?? "true");
            ...
            else if (forClause is not null)
            {
                ...
                if (forClause.expression() is not null)
                {
                    if (Expressions.TryGetValue(forClause.expression(), out ExpressionInfo expression))
                        m_targetOutputFile.Replace(string.Format(ForExpressionMarker, m_forExpressionLevel), expression.Text);
                    else
                        AddWarning(context, $"Failed to find expression in for statement: {context.GetText()}");
                }
                else
                {
                    m_targetOutputFile.Replace(string.Format(ForExpressionMarker, m_forExpressionLevel), string.Empty);
                }
                ...
                if (hasInitStatement)
                {
                    if (m_simpleStatements.TryGetValue(simpleInitStatement, out string statement))
                    {
                        ...
                        m_targetOutputFile.Replace(string.Format(ForInitStatementMarker, m_forExpressionLevel), statement);
                    }
                    else
                    {
                    ...
                    if (isRedeclared || !useForStyleStatement && simpleInitStatement.shortVarDecl() is not null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(closeRedeclarations))
                            m_targetOutputFile.Append($"{Environment.NewLine}{RemoveLastLineFeed(closeRedeclarations)}");
                ...
                if (hasPostStatement)
                {
                    if (m_simpleStatements.TryGetValue(simplePostStatement, out string statement))
                    {
                        ...
                        m_targetOutputFile.Replace(string.Format(ForPostStatementMarker, m_forExpressionLevel), statement);
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (rangeClause is not null)
                {
                    ...
                    if (expressions is not null)
                    {
                        ...
                        m_targetOutputFile.Replace(string.Format(ForRangeExpressionsMarker, m_forExpressionLevel), $"var ({string.Join(", ", immutable)})");
                        m_targetOutputFile.Replace(string.Format(ForRangeBlockMutableExpressionsMarker, m_forExpressionLevel), mutable.ToString());
                    }
                    ...
                    if (identifiers is not null)
                    {
                        ...
                        if (isRedeclared)
                        {
                            ...
                            if (!string.IsNullOrEmpty(closeRedeclarations))
                                m_targetOutputFile.Append(RemoveLastLineFeed(closeRedeclarations));

                            IndentLevel--;
                            m_targetOutputFile.AppendLine();
                            m_targetOutputFile.AppendLine($"{Spacing()}}}{Environment.NewLine}");
                        ...
                        {
                            m_targetOutputFile.Replace(string.Format(ForRangeExpressionsMarker, m_forExpressionLevel), $"var ({string.Join(", ", identifiers)})");
                            m_targetOutputFile.Replace(string.Format(ForRangeBlockMutableExpressionsMarker, m_forExpressionLevel), string.Empty);
                        }
                    }
