...
public partial class ConvASqlModels
{
    public override void EnterMethodDecl(GoParser.MethodDeclContext context)
    {
        ...
        m_targetOutputFile.Append($"{Spacing()}{m_functionResultTypeMarker} {CurrentFunctionName}{m_functionParametersMarker}{m_functionExecContextMarker}");

        if (Options.UseAnsiBraceStyle)
            m_targetOutputFile.AppendLine();

        if( isPublicByName(CurrentFunction.Signature.Name ) 
          && Options.ConvertWebApi.bCollectMethods ) 
        {
            this.m_exportedMethods.Add( CurrentFunction);
        }
    }
    ...
    public override void ExitMethodDecl(GoParser.MethodDeclContext context)
    {
        ...
        goscanner.Metadata.ParameterInfo[] receiverParameters = method.ReceiverParameters ?? Array.Empty<goscanner.Metadata.ParameterInfo>();
        ...
        foreach (goscanner.Metadata.ParameterInfo parameter in signature.Result)
        {
        ...
        foreach (goscanner.Metadata.ParameterInfo parameter in receiverParameters.Concat(signature.Parameters))
        {
        ...
        m_targetOutputFile.Replace(m_functionResultTypeMarker, resultSignature);
        m_targetOutputFile.Replace(m_functionParametersMarker, parametersSignature);

        if (useFuncExecutionContext)
        {
            Stack<string> unusedNames = new(new[] { Options.WriteLegacyCompatibleCode ? "__" : "_", "_" });
            m_targetOutputFile.Replace(m_functionExecContextMarker, $" => func(({(hasDefer ? "defer" : unusedNames.Pop())}, {(hasPanic ? "panic" : unusedNames.Pop())}, {(hasRecover ? "recover" : unusedNames.Pop())}) =>");
        }
        else
        {
            m_targetOutputFile.Replace(m_functionExecContextMarker, string.Empty);
        }

        m_targetOutputFile.Replace(string.Format(FunctionBlockPrefixMarker, CurrentFunctionName), blockPrefix);
