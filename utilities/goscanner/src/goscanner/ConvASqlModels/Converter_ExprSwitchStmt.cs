...
public partial class ConvASqlModels
{
        ...
        { 
            if (context.simpleStmt() is not null && context.simpleStmt().emptyStmt() is null)
            {
                if (context.simpleStmt().shortVarDecl() is not null)
                {
                    ...
                    m_targetOutputFile.Append(OpenRedeclaredVariableBlock(context.simpleStmt().shortVarDecl().identifierList(), m_exprSwitchExpressionLevel));
                }

                m_targetOutputFile.Append(string.Format(ExprSwitchStatementMarker, m_exprSwitchExpressionLevel));
            }
    ...
    public override void ExitExprCaseClause(GoParser.ExprCaseClauseContext context)
    {
        ...
        if (EMITTING_CODE)
        {
            ...
            m_fallThrough = false;
            m_fallThroughComment = string.Empty;
        ...
        if (EMITTING_CODE)
        {
            ...
            if (exprSwitchStatement.anyFallthroughs)
            {
                ...
                if (!string.IsNullOrEmpty(exprSwitchStatement.intraSwitchComments))
                    m_targetOutputFile.Append(FixForwardSpacing(exprSwitchStatement.intraSwitchComments));

                for (int i = 0; i < caseStatements.Length; i++)
                {
                    ExprCaseStatement caseStatement = caseStatements[i];
                    
                    m_targetOutputFile.Append($"{Spacing()}if (");

                    if (i > 0)
                    {
                        if (caseStatements[i - 1].hasFallthrough)
                            m_targetOutputFile.Append("fallthrough || ");
                    }
                    ...
                    for (int j = 0; j < caseStatement.expressionList.Length; j++)
                    {
                        ...
                        m_targetOutputFile.Append($"{expressionText}{caseExpr.Text}");
                    }

                    (string eolComment, string comments) = SplitEOLComment(caseStatement.leftComments);
                    m_targetOutputFile.Append($"){eolComment}{Environment.NewLine}{Spacing()}{{{Environment.NewLine}{comments}{caseStatement.block}");
                    if (caseStatement.hasFallthrough)
                    {
                        if (i < caseStatements.Length - 1)
                            m_targetOutputFile.Append($"{Spacing(1)}fallthrough = true;{caseStatement.fallthroughComment}");
                    }
                    else
                    {
                        m_targetOutputFile.Append($"{Spacing(1)}goto {breakLabel};{Environment.NewLine}");
                    }

                    m_targetOutputFile.Append($"{Spacing()}}}{Environment.NewLine}");
                }

                if (exprSwitchStatement.defaultCase is not null)
                    m_targetOutputFile.Append($"{Spacing()}// default:{FixForwardSpacing(exprSwitchStatement.defaultCase.leftComments, 1, firstIsEOLComment: true)}{exprSwitchStatement.defaultCase.block}");

                m_targetOutputFile.Append($"{Environment.NewLine}{Spacing()}{breakLabel}:;{(hasSwitchStatement ? Environment.NewLine : CheckForCommentsRight(context))}");
            }
            else if (exprSwitchStatement.allConst && expression is not null)
            {
                ...
                m_targetOutputFile.Append($"{Spacing()}switch ({expression.Text}){(Options.UseAnsiBraceStyle ? $"{Environment.NewLine}{Spacing()}" : " ")}{{{RemoveLastLineFeed(exprSwitchStatement.intraSwitchComments)}");
                ...
                foreach (ExprCaseStatement caseStatement in exprSwitchStatement.caseStatements.Values)
                {
                    foreach (ExpressionInfo caseExpr in caseStatement.expressionList)
                        m_targetOutputFile.AppendLine($"{Environment.NewLine}{Spacing(1)}case {caseExpr.Text}:{RemoveLastLineFeed(FixForwardSpacing(caseStatement.leftComments, 2, firstIsEOLComment: true))}");

                    m_targetOutputFile.Append(FixForwardSpacing(caseStatement.block, 2));

                    if (!EndsWithLineFeed(caseStatement.block))
                        m_targetOutputFile.AppendLine();

                    m_targetOutputFile.Append($"{Spacing(2)}break;");
                }

                if (exprSwitchStatement.defaultCase is not null)
                {
                    m_targetOutputFile.AppendLine($"{Environment.NewLine}{Spacing(1)}default:{RemoveLastLineFeed(FixForwardSpacing(exprSwitchStatement.defaultCase.leftComments, 2, firstIsEOLComment: true))}");
                    m_targetOutputFile.Append(FixForwardSpacing(exprSwitchStatement.defaultCase.block, 2));
                ...
                m_targetOutputFile.Append($"{Environment.NewLine}{Spacing()}}}{(hasSwitchStatement ? Environment.NewLine : CheckForCommentsRight(context))}");
            }
            else
            {
                ...
                for (int i = 0; i < caseStatements.Length; i++)
                {
                    ...
                    m_targetOutputFile.Append($"{Spacing()}{(i > 0 ? "else " : string.Empty)}if (");
                    ...
                    m_targetOutputFile.Append($"){FixForwardSpacing(caseStatement.leftComments, 1, firstIsEOLComment: true)}{caseStatement.block}");
                }

                if (exprSwitchStatement.defaultCase is not null)
                {
                    m_targetOutputFile.Append($"{Spacing()}else{FixForwardSpacing(exprSwitchStatement.defaultCase.leftComments, 1, firstIsEOLComment: true)}{exprSwitchStatement.defaultCase.block}");
                }

                if (hasSwitchStatement)
                    m_targetOutputFile.AppendLine();
                else
                    m_targetOutputFile.Append($"{Spacing()}{CheckForCommentsRight(context).TrimStart()}");
            }

            if (hasSwitchStatement)
            {
                if (m_simpleStatements.TryGetValue(context.simpleStmt(), out string statement))
                    m_targetOutputFile.Replace(string.Format(ExprSwitchStatementMarker, m_exprSwitchExpressionLevel), $"{statement}{Environment.NewLine}");
                else
                ...
                if (context.simpleStmt().shortVarDecl() is not null)
                {
                    ...
                    m_targetOutputFile.Append(CloseRedeclaredVariableBlock(context.simpleStmt().shortVarDecl().identifierList(), m_exprSwitchExpressionLevel));
