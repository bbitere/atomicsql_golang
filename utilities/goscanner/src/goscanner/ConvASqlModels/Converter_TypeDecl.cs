...
public partial class ConvASqlModels
{
    ...
    public override void ExitTypeDecl(GoParser.TypeDeclContext context)
    {
        ...
        if (m_typeMultipleDeclaration && EndsWithLineFeed(m_targetOutputFile.ToString()))
        {
    ...
    public override void ExitTypeSpec(GoParser.TypeSpecContext context)
    {
        ...
        if (Metadata.Interfaces.TryGetValue(originalIdentifier, out InterfaceInfo interfaceInfo))
        {
            ...
            if( EMITTING_CODE )
            using (StreamWriter writer = File_CreateText(ancillaryInterfaceFileName))
            {
                    /*
            ...
            m_targetOutputFile.Append($"{Spacing()}{scope} partial interface {identifier}{inheritedInterfaces}");

            //if( WebApiBase_Go.Class_Generic_MODEL == identifier)
            //{
            //    s_exist_CLASS_GenericModel = true;
            //}
            ...
            {
                m_targetOutputFile.Append($"{Spacing(1)}{function.Signature.GenerateResultSignature()} {SanitizedIdentifier(function.Name)}({function.Signature.GenerateParametersSignature()});{(string.IsNullOrWhiteSpace(function.Comments) ? Environment.NewLine : $" {function.Comments.TrimStart()}")}");
            }
        ...
        else if (Metadata.Structs.TryGetValue(originalIdentifier, out StructInfo structInfo))
        {
            ...
            s_AllStructs[structInfo.Name] = structInfo;

            //if(structInfo.Name.Contains( "User"))
            //    Debugger.Break();

            if( structInfo.Comment != null 
             && structInfo.Comment.Trim().StartsWith($"/*{OrmDef.Atomicsql_table}\""))
            {
                this.m_models.Add( structInfo );
            }else
            if ( isInheritedBy( structInfo, GenerateJsonModels.Class_Generic_MODEL, inheritedTypeNames ) )
            {
                this.m_models.Add( structInfo );
            }
            ...
            if ( EMITTING_CODE )
            { 
                using (StreamWriter writer = File_CreateText(ancillaryStructFileName))
                {
            ...
            m_targetOutputFile.Append($"{Spacing()}{scope} partial struct {identifier}{GetInheritedTypeList(inheritedTypeNames)}");
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                m_targetOutputFile.Append(fieldDecl);
            }
        ...
        else if (Types.TryGetValue(context.type_(), out TypeInfo typeInfo))
        {
            if (typeInfo.TypeClass == TypeClass.Function)
            {
                ...
                {
                    m_targetOutputFile.Append($"{Spacing()}public delegate void {identifier}();");
                ...
                else if (signature.StartsWith("Action<", StringComparison.Ordinal))
                {
                    ...
                    m_targetOutputFile.Append($"{Spacing()}public delegate void {identifier}({signature});");
                    m_targetOutputFile.Append(CheckForCommentsRight(context));
                }
                else if (signature.StartsWith("Func<", StringComparison.Ordinal))
                {
                    ...
                    if (parts.Length > 0)
                    {
                        ...
                        m_targetOutputFile.Append($"{Spacing()}public delegate {result} {identifier}({signature});");
                        m_targetOutputFile.Append(CheckForCommentsRight(context));
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (EMITTING_CODE)
                using (StreamWriter writer = File_CreateText(ancillaryInheritedTypeFileName))
                {
                ...
                m_targetOutputFile.Append($"{Spacing()}{scope} partial struct {identifier}");

                string typeName = typeInfo.TypeName;

                s_AllTypeDefs[ identifier ] = typeInfo;
                ...
                if (Options.UseAnsiBraceStyle)
                {
                    m_targetOutputFile.AppendLine(typeName);
                    m_targetOutputFile.AppendLine($"{Spacing()}{{");
                }
                else
                {
    ...
    private void SearchPromotedStructFields(GoParser.TypeSpecContext context, string identifier, StructInfo structInfo, HashSet<string> inheritedTypeNames, Dictionary<string, List<FieldInfo>> promotedFields, HashSet<string> promotedStructTypeNames = null, bool useFullTypeName = false)
    {
        foreach (FieldInfo field in structInfo.GetAnonymousFields())
        {
            ...
            {
                //AddWarning(context, $"declare a name for struct \"{structName}\" declared in \"{identifier}\" struct type. ");
            }
        }
    }
